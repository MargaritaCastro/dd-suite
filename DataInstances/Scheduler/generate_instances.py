...
Generation scheme follows the standard used in the scheduling literature
...
Output format (SchedulerInstance-compatible):
  n_jobs
  p_0 p_1 ... p_{n-1}
  w_0 w_1 ... w_{n-1}
...
if __name__ == "__main__":
    ...
    for n in SIZES:
        for sigma in SIGMAS:
            for seed in range(1, SEEDS_PER_CONFIG + 1):
                ...
                tag = sigma_tag(sigma)
                filename = f"scheduler_n{n}_s{tag}_seed{seed}.txt"
    ...
    print()
    print("File naming: scheduler_n<n>_s<sigma>_seed<k>.txt")
