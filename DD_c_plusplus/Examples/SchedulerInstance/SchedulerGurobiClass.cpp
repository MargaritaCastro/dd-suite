#include "SchedulerGurobiClass.h"
#include <numeric>
#include <algorithm>
#include <iomanip>
#include <sstream>

SchedulerGurobiClass::SchedulerGurobiClass(const SchedulerInstance& params, bool continuous)
...
double SchedulerGurobiClass::big_M() const {
    double M = 0;
    for (int j : p_) M += j;
    for (int j = 0; j < n_; ++j) {
        int max_setup = 0;
...
void SchedulerGurobiClass::initialize_variables() {
    double M = big_M();
...
void SchedulerGurobiClass::set_objective() {
    GRBLinExpr obj = 0;
    for (int j = 0; j < n_; ++j)
        obj += w_[j] * C_[j];
...
void SchedulerGurobiClass::add_constraints() {
    double M = big_M();
...
void SchedulerGurobiClass::optimize_with_cuts(
    vector<DD<SchedulerState>*> /*dds*/, CutType /*cut_type*/, bool /*cut_strengthening*/)
{
...
void SchedulerGurobiClass::show_results() {
