#include "SchedulerPathSolver.h"
#include <iostream>
#include <algorithm>
#include <sstream>

SchedulerPathSolver::SchedulerPathSolver(DD<SchedulerState>& dd)
    : graph_(dd.get_decision_diagram()),
      problem_(dynamic_cast<const SchedulerProblem*>(&dd.problem)),
      time_(chrono::duration<double>(0))
{}

void SchedulerPathSolver::set_parameters(const vector<double>& /*weights*/, const string& objective) {
    objective_ = objective;
}

PathStructureSolution<SchedulerState> SchedulerPathSolver::solve() {
...
PathStructureSolution<SchedulerState> SchedulerPathSolver::get_solution() const {
    return solution_;
}

string SchedulerPathSolver::get_time() const {
    return to_string(time_.count());
}

void SchedulerPathSolver::run_dp() {
    double cost_sign = (objective_ == "max") ? -1.0 : 1.0;
    vector<NodeDP> dp_data;
    vector<Arc<SchedulerState>*> dp_arcs;
    initialize_dp_tables(dp_data, dp_arcs);
    forward_pass(dp_data, dp_arcs, cost_sign);
    extract_solution(dp_data, dp_arcs);
}

void SchedulerPathSolver::initialize_dp_tables(
    vector<NodeDP>& dp_data, vector<Arc<SchedulerState>*>& dp_arcs)
{
    int total_nodes = graph_->get_node_count();
    dp_data.resize(total_nodes);
    dp_arcs.assign(total_nodes, nullptr);
    Node<SchedulerState>* root_node = graph_->structure.front().front();
...
void SchedulerPathSolver::forward_pass(
    vector<NodeDP>& dp_data, vector<Arc<SchedulerState>*>& dp_arcs, double cost_sign)
{
    int total_weight = 0;
    for (int w : problem_->weights) total_weight += w;

    for (size_t layer_idx = 1; layer_idx < graph_->structure.size(); ++layer_idx) {
        for (Node<SchedulerState>* node : graph_->structure[layer_idx]) {
            int node_id = node->get_id();
            for (Arc<SchedulerState>* arc : node->in_arcs) {
                ...
                if (!current.valid || candidate_cost < current.cost) {
                    ...
                    current.scheduled_weight = parent.scheduled_weight + problem_->weights[job_id];
                    current.valid            = true;
                    dp_arcs[node_id] = arc;
                }
            }
...
void SchedulerPathSolver::extract_solution(
    const vector<NodeDP>& dp_data, const vector<Arc<SchedulerState>*>& dp_arcs)
{
    Node<SchedulerState>* terminal_node = graph_->structure.back().front();
...
void SchedulerPathSolver::backtrack_optimal_path(
    const vector<Arc<SchedulerState>*>& dp_arcs, int terminal_id)
{
    solution_.path_arcs.clear();
    string path_string;
    int current_id = terminal_id;
    while (dp_arcs[current_id] != nullptr) {
        Arc<SchedulerState>* arc = dp_arcs[current_id];
