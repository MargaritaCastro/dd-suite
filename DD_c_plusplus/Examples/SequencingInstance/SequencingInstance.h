#ifndef DD_OPTIMIZATION_PROGRAM_SEQUENCINGINSTANCE_H
#define DD_OPTIMIZATION_PROGRAM_SEQUENCINGINSTANCE_H
...
/**
 * @brief Reads a single-machine sequencing instance from file.
 *
 * File format:
 *   n_jobs
 ...
 */
class SequencingInstance {
public:
    explicit SequencingInstance(const string& file_path);
    ...
    SequencingState*                initial_state;

    ~SequencingInstance() { delete initial_state; }

private:
    void read_file(const string& file_path);
};

#endif //DD_OPTIMIZATION_PROGRAM_SEQUENCINGINSTANCE_H
