...
//! diagram for the single-machine scheduling problem using DDO.
//!
...
//! cost of scheduling `job` from a partial sequence is
...
//!
//! Mirrors the Python (`SchedulerProblem`) and C++ (`SchedulerProblem`)
...
/// DP state for the single-machine scheduling problem.
...
/// the state. This matches the C++ (`SchedulerState`) and Python
...
impl Scheduler {
    pub fn new(
        ...
        Scheduler { n_jobs, processing_times, weights, setup_times, total_weight }
    }
...
impl Problem for Scheduler {
    ...
    fn initial_state(&self) -> Self::State {
        SchedulerState { scheduled: 0, last_job: DEPOT }
    }
    ...
    /// Separable contribution of scheduling `job` from the source state, negated
...
pub struct SchedulerRelax<'a> {
    pb: &'a Scheduler,
}

impl Relaxation for SchedulerRelax<'_> {
    type State = SchedulerState;
    ...
    fn merge(&self, states: &mut dyn Iterator<Item = &Self::State>) -> Self::State {
        ...
        SchedulerState { scheduled: merged_sched, last_job: best_last }
    }
...
pub struct SchedulerRanking<'a> {
    pb: &'a Scheduler,
}

impl StateRanking for SchedulerRanking<'_> {
    type State = SchedulerState;
...
///   ... (n_jobs + 1 rows total)
/// ```
pub fn read_instance<P: AsRef<Path>>(fname: P) -> Scheduler {
    ...
    Scheduler::new(n_jobs, processing_times, weights, setup_times)
}
...
fn main() {
    ...
    if args.len() != 5 {
        eprintln!(
            "Uso: cargo run --example compare_scheduler \
    ...
    let file_path = format!("../resources/scheduler/{}", input_file);
    let problem = read_instance(&file_path);
    let relaxation = SchedulerRelax { pb: &problem };
    let ranking = SchedulerRanking { pb: &problem };
    ...
    let mut dd = Mdd::<SchedulerState, { FRONTIER }>::new();
    ...
    match result {
        Ok(completion) => {
            ...
            // Maximisamos el costo negado, así que el óptimo real es -best_value.
            let best_value_str =
