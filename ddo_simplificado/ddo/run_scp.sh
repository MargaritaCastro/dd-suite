...
OUTPUT_FILE="../results_rust_scp.csv"

FILES=()
while IFS= read -r -d '' file; do
    FILES+=("$(basename "$file")")
done < <(find ../resources/scp/ -type f -print0 | sort -z)
    ...
    for COMP_TYPE in "${TIPOS[@]}"; do
        if [[ "$COMP_TYPE" == "Restricted" || "$COMP_TYPE" == "Relaxed" ]]; then
            for MAX_WIDTH in "${WIDTHS[@]}"; do
                ...
                cargo run --example compare_scp -- "$INPUT_FILE" "$OUTPUT_FILE" "$COMP_TYPE" "$MAX_WIDTH"
            done
        else 
            # Exact no necesita width, se entrega un dummy por compatibilidad
            echo "🚀 Ejecutando con archivo: $INPUT_FILE, COMP_TYPE: $COMP_TYPE"
            cargo run --example compare_scp -- "$INPUT_FILE" "$OUTPUT_FILE" "$COMP_TYPE" 99999
        fi
    done
done
