...
WIDTHS=(100)
OUTPUT_FILE="../results_rust_misp.csv"

FILES=()
while IFS= read -r -d '' file; do
    FILES+=("$(basename "$file")")
done < <(find ../resources/misp/ -type f -print0 | sort -z)
    ...
    for COMP_TYPE in "${TIPOS[@]}"; do
        if [[ "$COMP_TYPE" == "Restricted" || "$COMP_TYPE" == "Relaxed" ]]; then
            for MAX_WIDTH in "${WIDTHS[@]}"; do
                ...
                cargo run --example compare_misp -- "$INPUT_FILE" "$OUTPUT_FILE" "$COMP_TYPE" "$MAX_WIDTH"
            done
        else 
            # Exact does not need a width; a dummy is passed for compatibility
            echo "🚀 Running with file: $INPUT_FILE, COMP_TYPE: $COMP_TYPE"
            cargo run --example compare_misp -- "$INPUT_FILE" "$OUTPUT_FILE" "$COMP_TYPE" 99999
        fi
    done
done
