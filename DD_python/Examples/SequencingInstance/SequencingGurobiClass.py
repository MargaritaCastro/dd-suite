...
class SequencingGurobi(AbstractProblemGurobi):
    """
    MIP formulation for the single-machine sequencing problem minimising Σ w_j * C_j
    ...
    def __init__(self, parameters: 'SequencingStructure', continuous: bool = False) -> None:
    ...
    def _big_M(self) -> float:
        return sum(self.p) + sum(
            max(self.s[k][j] for k in range(self.n + 1) if k != j + 1) for j in range(self.n)
        )
    ...
    # ------------------------------------------------------------------
    # FlowCuts / JointFlowCuts require a BDD; the Sequencing DD is an MDD.
    ...
    def optimize_with_cuts(self, dds, cut_type=None, cut_strengthening=False) -> None:
        if cut_type != CutType.TARGET:
            print(f"Note: Only TargetCuts is valid for the Sequencing (got {cut_type}). Running plain Gurobi.")
        self.optimize_model()
