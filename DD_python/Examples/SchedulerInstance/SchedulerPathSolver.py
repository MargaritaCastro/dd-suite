...
class SchedulerPathSolver:
    """
    DP-based shortest/longest path solver for the single-machine scheduling problem.
    ...
    The forward DP still tracks last_job and the accumulated scheduled weight
    along each path (instead of the time), so it stays correct even on reduced /
    merged diagrams, where a node's stored state may not match a particular path.
    ...
    def _initialize_dp_tables(self):
        ...
        dp_data = [None] * total_nodes  # (cost, last_job, scheduled_weight)
        dp_arcs = [None] * total_nodes
        root_node = self._graph.structure[0][0]
        dp_data[root_node.get_id()] = (0, -1, 0)
            ...
            for node in layer:
                ...
                for arc in node.in_arcs:
                    ...
                    if dp_data[parent_id] is None:
                        continue
                    parent_cost, parent_last, parent_weight = dp_data[parent_id]
                    ...
                    if dp_data[node_id] is None or candidate_cost < dp_data[node_id][0]:
                        new_weight = parent_weight + self._problem.weights[job_id]
                        dp_data[node_id] = (candidate_cost, job_id, new_weight)
                        dp_arcs[node_id] = arc
